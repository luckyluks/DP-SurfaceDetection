...
from ternausnet.validation import validation_binary, validation_multi
...
from ternausnet.models import UNet11 , LinkNet34, UNet, UNet16, AlbuNet
from ternausnet.loss import LossBinary, LossMulti
from ternausnet.dataset import RoboticsDataset
from ternausnet import utils
from ternausnet import getFilenames
import sys
...
def main(devid, bsize, fold, work, lr, nep, jcwe, modelc, th, tw, vh, vw):
    parser = argparse.ArgumentParser()
    arg = parser.add_argument
    arg('--jaccard-weight', default=jcwe, type=float)
    arg('--device-ids', type=str, default=devid, help='For example 0,1 to run on two GPUs')
    arg('--fold', type=int, help='fold', default=fold)
    arg('--root', default='data', help='checkpoint root')
    arg('--batch-size', type=int, default=bsize)
    arg('--n-epochs', type=int, default=nep)
    arg('--lr', type=float, default=lr)
    arg('--workers', type=int, default=work)
    arg('--train_crop_height', type=int, default=th)
    arg('--train_crop_width', type=int, default=tw)
    arg('--val_crop_height', type=int, default=vh)
    arg('--val_crop_width', type=int, default=vw)
    arg('--type', type=str, default='binary', choices=['binary', 'parts', 'instruments'])
    arg('--model', type=str, default=modelc, choices=moddel_list.keys())
    ...
    train_file_names, val_file_names = getFilenames.get_names()
